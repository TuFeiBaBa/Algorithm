...
*					即对每个点自身的x、y进行合并。(不需要考虑怎么合并两个点。两个点只要x/y有重叠，按并查集的逻辑，最终肯定会merge)
*		3.怎么区分x、y坐标：石头的位置是「有序数对（二维）」，并查集的底层是「一维数组」，我们在并查集里应该如何区分横纵坐标呢？
*		4.count怎么计数：x、y单独算。
			1)合并一个点的x、y时，如果x、y都是第一次出现，必有两次count++。经过merge后，会count--。最终count加1，即只会新增1个连通分量。
			2)合并一个点的x、y时，如果x、y都已出现过
				- 之前x、y没有合并，会进行merge，count--。即连通分量减一。
				- 之前x、y已合并，rootX必然等于rootY，不会进行实际的merge。连通分量不会发生增减。
			3)合并一个点的x、y时，如果x、y其一是新出现的，一次count++。然后merge后，count--。连通分量不会发生增减。
*
*/
class Solution {
private:
