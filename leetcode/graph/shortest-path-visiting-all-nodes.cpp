...
*	  所以拓扑排序（无环）、关键路径（无环）、Dijkstra（单源且边不可重复经过）、Bellman-Ford（单源且边不可重复经过）等算法都不适用
...
*	  因此可以看成是进行了 2^n 次常规的广度优先搜索（这里说法太跳跃了，无法理解）。由于 m 的范围没有显式给出，在最坏情况下为完全图，利用组合可知，有n(n - 1)/2条边，
